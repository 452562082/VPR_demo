    ...
    callWriteData(false)
{
    ...
    QObject::connect(&listener, SIGNAL(newConnection()), SLOT(setupSocket()));
}
...
void QFtpDTP::connectToHost(const QString & host, quint16 port)
{
    ...
    QObject::connect(socket, SIGNAL(connected()), SLOT(socketConnected()));
    QObject::connect(socket, SIGNAL(readyRead()), SLOT(socketReadyRead()));
    QObject::connect(socket, SIGNAL(error(QAbstractSocket::SocketError)), SLOT(socketError(QAbstractSocket::SocketError)));
    QObject::connect(socket, SIGNAL(disconnected()), SLOT(socketConnectionClosed()));
    QObject::connect(socket, SIGNAL(bytesWritten(qint64)), SLOT(socketBytesWritten(qint64)));
    ...
    waitForDtpToClose(false)
{
    ...
    QObject::connect(&commandSocket, SIGNAL(hostFound()),
            SLOT(hostFound()));
    QObject::connect(&commandSocket, SIGNAL(connected()),
            SLOT(connected()));
    QObject::connect(&commandSocket, SIGNAL(disconnected()),
            SLOT(connectionClosed()));
    QObject::connect(&commandSocket, SIGNAL(readyRead()),
            SLOT(readyRead()));
    QObject::connect(&commandSocket, SIGNAL(error(QAbstractSocket::SocketError)),
            SLOT(error(QAbstractSocket::SocketError)));

    QObject::connect(&dtp, SIGNAL(connectState(int)),
    ...
    : QObject(parent), d(new QFtpPrivate(this))
{
    ...
    QObject::connect(&d->pi, SIGNAL(connectState(int)),
            SLOT(_q_piConnectState(int)));
    QObject::connect(&d->pi, SIGNAL(finished(QString)),
            SLOT(_q_piFinished(QString)));
    QObject::connect(&d->pi, SIGNAL(error(int,QString)),
            SLOT(_q_piError(int,QString)));
    QObject::connect(&d->pi, SIGNAL(rawFtpReply(int,QString)),
            SLOT(_q_piFtpReply(int,QString)));

    QObject::connect(&d->pi.dtp, SIGNAL(readyRead()),
            SIGNAL(readyRead()));
    QObject::connect(&d->pi.dtp, SIGNAL(dataTransferProgress(qint64,qint64)),
            SIGNAL(dataTransferProgress(qint64,qint64)));
    QObject::connect(&d->pi.dtp, SIGNAL(listInfo(QUrlInfo)),
